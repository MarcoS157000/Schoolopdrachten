=== Busyness ===

Contributors: wenthemes
Requires at least: 5.1
Tested up to: 5.5
Requires PHP: 5.6
Stable tag: 1.1.7
License: GPLv3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

A Multipurpose Corporate WordPress theme with an aesthetically pleasant and modern looking design.

== Description ==

Busyness, with its aesthetically pleasant and modern looking design, is a Multipurpose Corporate WordPress theme fully capable of handling the needs and requirements of a vast range of corporate and portfolio websites. The theme is beautifully crafted, clean, easy to use features, and responsive. It has been designed for everybody with or without previous coding experience to effectively and helps to speedily put together polished, professional-quality websites without having to so much as peek at a single line of code. Official Support Forum: https://themepalace.com/support-forum/. Full Demo: https://wenthemes.com/theme-demos/?demo=busyness

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Changelog ==

= 1.1.7 - Sep 30 2020 =
* Fix: Footer Text now takes post HTML

= 1.1.6 - Sep 24 2020 =
* Fix: Comment form focus style

= 1.1.5 - Sep 20 2020 =
* Fix: Keyboard navigation
* Fix: Focus issue in input, textarea and select focus style

= 1.1.4 - Sep 11 2020 =
* Fix: jQuery Migrate hover issue

= 1.1.3 - Sep 07 2020 =
* Fix: Typo issue in Widget

= 1.1.2 - Sep 07 2020 =
* Fix: HTML structure for title

= 1.1.1 - Sep 03 2020 =
* Fix: Header height in mobile
* Fix: Border-radius support for ios

= 1.1 - Sep 02 2020 =
* Fix: Hero content display in mobile
* Fix: Slider button margin issue
* Fix: Add missing font rem value
* Fix: Logo max-height issue in mobile
* Fix: Metabox capability check issue

= 1.0.5 - Aug 12 2020 =
* Update: Readme

= 1.0.4 - Jul 14 2020 =
* Fix: Escaping issue
* Fix: Added stable tag
* Fix: Screenshot image

= 1.0.3 - Jul 06 2020 =
* Add: Missing resources
* Update: Code to check for capabilities on metabox save
* Update: Homepage Settings => Enable Content on by default

= 1.0.2 - Jun 29 2020 =
* Fix: CSS Cleanup and improve Keyboard Navigation
* Fix: Move date in single to header

= 1.0.1 - Jun 23 2020 =
* Fix: Excerpt settings
* Fix: Header title HTML issue
* Fix: Archive Description issue in header
* Fix: Header Image issue in Frontpage
* Fix: Escaping Issues
* Fix: Menu color issue in focus
* Fix: Mobile Menu Keyboard Navigation
* Update: Customizer settings
* Update: Screenshot

= 1.0 - May 31 2020 =
* Initial release

== Credits ==

Underscores
	Based on Underscores http://underscores.me/, (C) 2012-2016 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

Normalize
	normalize.css http://necolas.github.io/normalize.css/, (C) 2012-2016 Nicolas Gallagher and Jonathan Neal, [MIT](http://opensource.org/licenses/MIT)

SVG Icons
	SVG Icons function https://github.com/WordPress/twentyseventeen (C) 2016 WordPress.org, [GNU GPL, version 2 or later ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

TGM Plugin Activation
	class-tgm-plugin-activation.php https://github.com/TGMPA/TGM-Plugin-Activation [GNU General Public License v2.0]

HTML5-Shiv is Licensed under the MIT and GPL licenses
    https://code.google.com/p/html5shiv/

Slick Version - 1.6.0
	Source: http://github.com/kenwheeler/slick
	License: The MIT License (MIT) copyright (c) 2013-2016
	license Url : https://github.com/kenwheeler/slick/blob/master/LICENSE

Simple Fontawesome IconPicker
	Source: https://github.com/aumkarthakur/simple-fontawesome-iconpicker
	License: MIT License Copyright (c) 2016 aumkarthakur
	license Url : https://github.com/aumkarthakur/simple-fontawesome-iconpicker/blob/master/LICENSE

Font Awesome 4.7.0 by @davegandy
	License: http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
	Source: http://fontawesome.io

Font Awesome font is licensed under the SIL OFL 1.1:
	License: https://github.com/FortAwesome/Font-Awesome#license
    http://scripts.sil.org/OFL

Chosen Select - 1.4.2
	Source: https://github.com/harvesthq/chosen
	License: The MIT License (MIT) copyright (c) 2011-2015
	license Url : https://github.com/harvesthq/chosen/blob/master/LICENSE.md

Breadcrumb Trail - 1.0.0
	Source: https://github.com/justintadlock/breadcrumb-trail (C) 2008 – 2015 Justin Tadlock
	License: GNU GPL, version 2 or later
	license Url : http://www.gnu.org/licenses/old-licenses/gpl-2.0.html

Jquery Match Height - 0.7.2
	Source: https://github.com/liabru/jquery-match-height
	License: The MIT License (MIT) Copyright (c) 2014 Liam Brummitt
	license Url : https://github.com/liabru/jquery-match-height/blob/master/LICENSE

Packery - 2.1.1
	Source: https://github.com/metafizzy/packery
	License: The MIT License (MIT) Copyright (c) 2014 Liam Brummitt
	license Url : https://packery.metafizzy.co/license.html

Magnific Popup - 1.1.0
	Source: http://dimsemenov.com/plugins/magnific-popup/
	License: The MIT License (MIT) Copyright (c) 2014 Liam Brummitt
	license Url : https://github.com/dimsemenov/Magnific-Popup/blob/master/LICENSE

== Images ==

All images are licensed under [CC0](http://creativecommons.org/publicdomain/zero/1.0/legalcode.txt)
* Header Image: https://stocksnap.io/photo/woman-mobile-Q93BUD2Z3Z

Unless otherwise specified, all the theme images are created by us and licensed under the same license as the theme is.
